...
async def auto_login(self, EventScanPayload=None, ScanStatus=None, event_stream=None,
    ...
    if hotReload:
        if await self.load_login_status(statusStorageDir,
            ...
            return
        await self.login(enableCmdQR=enableCmdQR, picDir=picDir, qrCallback=qrCallback, EventScanPayload=EventScanPayload,
                         ScanStatus=ScanStatus, event_stream=event_stream,
                         loginCallback=loginCallback, exitCallback=exitCallback)
